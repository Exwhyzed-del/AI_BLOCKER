...
if ($backups) {
    ...
    $cleanContent = $hostsContent | Where-Object {
        -not ($_ -match "127\.0\.0\.1" -and ($_ -match "chatgpt|openai|claude|anthropic|gemini|cohere|huggingface|mistral|stability|midjourney|perplexity|poe|copilot|cursor|codeium|tabnine|kite|ollama|localai" -or $_ -match "\.ai|\.dev"))
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $wslDistros) {
        foreach ($distro in $wslDistros) {
            ...
            if ($distroName -ne "") {
                ...
                $cmd = "sudo sed -i '/127\.0\.0\.1.*\(chatgpt\|openai\|claude\|anthropic\|gemini\|cohere\|huggingface\|mistral\|stability\|midjourney\|perplexity\|poe\|copilot\|cursor\|codeium\|tabnine\|kite\|ollama\|localai\|\.ai\)/d' /etc/hosts"
